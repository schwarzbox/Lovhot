...
local function paintPixels(imgdata,px,py,color)
    ...
    for x=1, sx do
        for y=1, sy do
            ...
            local clr = {r,g,b,a}
            if (r>0 or g>0 or b>0 or a>0) and (x<px or y<py) then
                clr = color
            end
...
local Tnt = Object:new{tag='Tnt', cnt=0, x=set.MIDWID,y=set.MIDHEI+64,
                        wid=twid,hei=thei, pivot=tpivot, color=set.GRAY,
                        particles={}}
...
function Tnt:new(o)
    ...
    self.initFire:setColors(set.RED,set.ORANGE,set.ORANGE0,
                         set.GRAY,set.WHITE0)
    ...
    self.tmr = {ntmr=0, timers={},
        ...
        update=function(s,dt)
            for tmr in pairs(s.timers) do
                local done = tmr(dt)
                if done then s.timers[tmr]=nil s.ntmr=s.ntmr-1 end
            end
        end,
        after=function(sec,finally)
            return function(t)
                if sec>0 then sec = sec - t return false end
                finally()
                return true
            end
        end,
...
function Tnt:update(dt)
    ...
    local data = paintPixels(bdata,
                    math.abs(self.fx-2-self.initAnim.x),
                    math.abs(self.fy-bpivot[2]-self.initAnim.y),set.GRAY64)
...
function Tnt:destroy()
    self.tmr:add('tween', 2, self.initAnim,'x', self.initAnim.x+4,
                 function() self:boom() end)
    self.tmr:add('tween', 2, self.initAnim,'y', self.initAnim.y+128,
                 function() self:boom() end)
...
function Tnt:draw()
    if not self.notnt then
        love.graphics.draw(tntimg,
                           self.x,self.y,0,1,1,
                           unpack(self.pivot))
        love.graphics.draw(bimg,
                           self.fx,self.fy,0,1,1,unpack(bpivot))
    end
